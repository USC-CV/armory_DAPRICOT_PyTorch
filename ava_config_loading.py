...
import logging
from ava_DAPRICOT_json_preprocess import *
...
def load_dataset(dataset_config, *args, num_batches=None, **kwargs):
    ...
    # Ava input config
    kwargs = {}
    for ds_kwarg in dataset_config:
        if ds_kwarg not in ['batch_size', "module", "name", "framework", "eval_split"]:
            kwargs[ds_kwarg] = dataset_config[ds_kwarg]
            if ds_kwarg in ["preprocessing_fn", "label_preprocessing_fn"]:
                kwargs[ds_kwarg] = eval(dataset_config[ds_kwarg])
